  ...
  it.only('판매 신청', () => {
    ...
    cy.findTableRowWithText(carNumber).then((matchingRows) => {
      if (matchingRows.length > 0) {
        ...
        // 매물 등록 버튼 클릭
        cy.get(firstMatchingRow).find('td:nth-child(4) button').contains('매물등록').click();
      } else {
    ...
    // 점검 정보 승인 페이지로 이동
    cy.wait(2000);
    cy.url().should('eq', BASE_URL + '/admin/sales/approve/60');

    // 이동한 점검 정보 승인 페이지의 차량 번호가 이전에 사용자가 신청한 차량 번호와 일치하는지 확인
    cy.get('h5.MuiTypography-root.MuiTypography-h5.css-rtyte8').first().invoke('text').then((text) => {
